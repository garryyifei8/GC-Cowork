  ...
  return (
    <div className="flex flex-col gap-4 p-6 h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-light-text">任务工作台</h1>
      ...
      {/* Stats bar */}
      <div className="flex items-center gap-3 text-sm bg-white border border-[#E8ECF4] rounded-[10px] px-5 py-3">
        <span className="text-light-text font-medium text-[15px]">
        ...
        {Object.entries(stats).map(([status, count]) => (
          <span
            ...
            style={{
              ...
              backgroundColor: `${TASK_STATUS_COLORS[status]}1A`,
            }}
          >
        ...
        {overdueCount > 0 && (
          <span className="text-xs px-2 py-1 rounded-[3px] font-medium text-[#E74C3C] bg-[#E74C3C1A]">
            逾期 {overdueCount}
          </span>
        )}
        {isLoading && <span className="text-xs text-[#919AA3] animate-pulse">加载中...</span>}
      </div>

      {/* Filters toolbar + Main view — wrapped in white card */}
      <div className="flex-1 min-h-0 bg-white border border-[#E8ECF4] rounded-[10px] p-5 flex flex-col gap-4 overflow-hidden">
      ...
      {selectedTaskIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 bg-[#475569] text-white rounded-xl shadow-2xl animate-slide-in-up">
          <span className="text-sm font-medium">已选 {selectedTaskIds.size} 项</span>
...
const BatchDropdown: React.FC<{
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute bottom-full left-0 mb-2 bg-white rounded-[10px] shadow-xl border border-[#E8ECF4] py-1 min-w-[100px]">
          {items.map((item) => (
            <button
              key={item.key}
              className="w-full text-left px-3 py-1.5 text-sm text-light-text hover:bg-[#F4F6FC] transition-colors"
              onClick={() => {
                onSelect(item.key);
                setOpen(false);
              }}
