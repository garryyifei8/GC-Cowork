    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-3 px-3 py-[9px] text-[15px] font-medium transition-all duration-150 rounded-lg mx-2 ${
          isActive ? 'bg-primary-tint text-primary' : 'text-light-text hover:bg-[#F4F6FC]'
        }`
      }
    >
...
const SectionHeader: React.FC<{ title: string }> = ({ title }) => (
  <div className="px-4 pt-4 pb-1.5">
    <span className="text-[12px] font-medium uppercase tracking-wider text-light-text-secondary">
      {title}
    </span>
  </div>
);
...
export const AppLayout: React.FC = () => {
  ...
  const getPageTitle = () => {
    ...
    if (pm) {
      const p = projects.find((pr) => pr.id === pm[1]);
      return p ? p.name : '项目详情';
    }
    const item = ALL_NAV.find((n) =>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F5F6FA]">
      ...
      {/* ============================================================== */}
      <aside
        className={`${SIDEBAR_W} flex flex-col bg-white border-r border-[#E8ECF4] shrink-0 transition-all duration-200 z-40`}
      >
        {/* Logo */}
        ...
        <div className="border-t border-[#E8ECF4] p-2.5 shrink-0">
          <NavLink
            to="/settings"
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-colors ${isActive ? 'bg-primary text-white' : 'text-light-text hover:bg-[#F4F6FC]'}`
            }
          >
            <Settings size={18} />
        ...
        <header className="flex items-center justify-between h-[68px] px-6 bg-white border-b border-white shrink-0 z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#EFF3F9] border-none h-[42px] text-sm text-light-text-secondary w-56">
              <Search size={14} />
              <span>搜索...</span>
              <kbd className="ml-auto px-1.5 py-0.5 text-xs rounded bg-white border border-[#E8ECF4] text-light-text-secondary">
                /
              </kbd>
            </div>
            ...
            <button
              ...
              className={`flex items-center justify-center w-9 h-9 rounded-lg transition-colors ${
                isPanelOpen ? 'bg-primary text-white' : 'text-light-text hover:bg-[#F4F6FC]'
              }`}
              title="AI小助理"
            >
              <Bot size={18} />
            ...
            <div className="relative" ref={notificationRef}>
              ...
              >
                <Bell size={18} />
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-danger text-white text-xs font-bold rounded-full flex items-center justify-center">
                    {unreadCount}
                  </span>
                )}
              </button>
