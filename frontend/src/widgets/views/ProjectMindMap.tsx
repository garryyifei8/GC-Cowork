...
const BRANCH_COLORS = [
  '#0086C0', '#6BBF59', '#9B51E0', '#FDAB3D', '#579BFC', '#FF7A59', '#37B4E3',
]
...
function buildMindMapTree(
  ...
  if (tasks.length > 0) {
    ...
    for (const t of tasks) { const s = t.status || 'todo'; (grouped[s] ??= []).push(t) }
  ...
  if (members.length > 0) {
    ...
    children.push(makeNode(
      ...
      teamChildren, members.length > 6 ? 1 : 0,
    ))
  }
  ...
  if (category === 'epc' && processes.length > 0) {
    ...
    for (const p of processes) { (grouped[p.record_type] ??= []).push(p) }
...
const ToolbarBtn: React.FC<{ title: string; onClick: () => void; children: React.ReactNode }> = ({ title, onClick, children }) => (
  <button
    title={title}
    onClick={onClick}
    className="p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-colors"
  >
    {children}
  </button>
)
...
const ProjectMindMap: React.FC<ProjectMindMapProps> = (props) => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="flex items-center gap-1 px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
        <span className="text-[0.9375rem] font-semibold mr-auto text-gray-800 dark:text-gray-100">思维导图</span>
