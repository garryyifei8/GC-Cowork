...
function getProjectIcon(projectType: string): { icon: React.ReactNode; color: string } {
  ...
  if (projectType.includes('信息化')) return { icon: <MonitorPlay size={16} />, color: '#E74C3C' };
...
const ProjectRow: React.FC<{ project: Project; onClick?: (p: Project) => void }> = ({
  project,
  onClick,
}) => {
  ...
  return (
    <tr
      className="border-t border-[#E8ECF4] hover:bg-[#EFF3F9] cursor-pointer transition-colors"
      onClick={handleClick}
      role="button"
      tabIndex={0}
    ...
    <tr>
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-light-text-secondary font-normal normal-case ml-1">
            ({projects.length})
          </span>
        </div>
      </td>
        ...
        <thead>
          <tr className="bg-[#E6FAF0]">
            {['项目名称', '类型', '状态', '进度', '截止日期', '预算', '团队'].map((heading) => (
              <th
                key={heading}
                className="px-5 py-[18px] text-left text-xs font-medium text-light-text-secondary uppercase tracking-wider"
              >
                {heading}
              </th>
            ))}
