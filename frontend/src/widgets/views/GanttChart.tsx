...
function getStatusColor(status: string): string {
  switch (status) {
    ...
    case 'completed': return '#919AA3'
    default: return '#2ED47E'
  }
}
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-[#E8ECF4] dark:border-gray-700 rounded-[10px] transition-colors duration-200 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-[#E8ECF4] dark:border-gray-700 bg-[#EFF3F9] dark:bg-gray-900/50">
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-200 mr-auto">甘特图</span>
            ...
            return (
              ...
              >
                <div className="w-48 shrink-0 px-4 py-3 border-r border-[#E8ECF4] dark:border-gray-700 flex flex-col justify-center gap-0.5">
                  <span className={`text-sm font-medium truncate ${isCritical ? 'text-red-600' : 'text-gray-800 dark:text-gray-100'}`}>
                    {isCritical && '🔴 '}{project.name}
                  </span>
                  <span className="text-xs text-[#919AA3] dark:text-gray-400">{project.status_label}</span>
                </div>
                <div className="flex-1 relative shrink-0" style={{ width: totalWidth, height: ROW_HEIGHT }}>
                  {/* Today marker */}
                  <div
          ...
          {showDeps && dependencies.length > 0 && (
            ...
            >
              ...
              {dependencies.map((dep, i) => {
                ...
                const marker = isCriticalDep && showCritical ? 'url(#arrowhead-critical)' : 'url(#arrowhead)'
      ...
      {showDeps && (
        <div className="flex items-center gap-4 px-4 py-2 border-t border-[#E8ECF4] dark:border-gray-700 text-xs text-[#919AA3]">
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-0.5 bg-[#919AA3] inline-block" style={{ borderTop: '1.5px dashed #919AA3' }} />
            依赖关系 (FS)
          </span>
          {showCritical && (
            <span className="flex items-center gap-1.5">
